...
mixer.music.load('FinalStage.mp3')
mixer.music.play()
...
enemy_hit_points = [7] * num_of_enemies
...
def show_score(x, y):
    score = font.render("Score : " + str(score_value) + " / 350 enemies defeated", True, (255, 255, 255))
...
while running:
    ...
    for i in range(num_of_enemies):
        if enemy_state[i]:
            ...
            if enemyX[i] <= 100:
                enemyX_change[i] = 8 # Change this value to a positive number
                enemyY[i] += enemyY_change[i]
            elif enemyX[i] >= 935:
                enemyX_change[i] = -8 # Change this value to a negative number
                enemyY[i] += enemyY_change[i]
    ...
    if score_value >= 350: 
        subprocess.Popen(["python", "SD_STAGE2.py"])
        running = False
